...
try {
	var localWebpackDevServer = require.resolve(path.join(process.cwd(), "node_modules", "webpacker-dev-server", "bin", "webpacker-dev-server.js"));
...
.usage("webpacker-dev-server " + require("../package.json").version + "\n" +
	"Usage: http://webpacker.github.io/docs/webpacker-dev-server.html")
...
.boolean("inline").describe("inline", "Inline the webpacker-dev-server logic into the bundle.")
...
if(options.inline) {
	...
	if(options.hot)
		devClient.push("webpacker/hot/dev-server");
	[].concat(wpOpt).forEach(function(wpOpt) {
...
new Server(webpack(wpOpt), options).listen(options.port, options.host, function(err) {
	...
	if(!options.inline)
		uri += "webpacker-dev-server/";

	if(err) throw err;
	console.log(" " + uri);
	console.log("webpacker result is served from " + options.publicPath);
